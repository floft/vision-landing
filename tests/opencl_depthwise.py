...
if __name__ == "__main__":
    ...
    weights = np.array([
        1, 2, 3, 4,
    ]).reshape((1, 2, 2, 1)).transpose((1,2,3,0)).astype(np.float32)
    bias = np.array([1]).astype(np.float32)
    stride = 1
    result_tf = depthwise_conv2d_tf(data, weights, bias, stride, Padding.VALID, np.float32)
    result = depthwise_conv2d_mine(data, weights, bias, stride, Padding.VALID, np.float32)
    print("TF")
    print(result_tf)
    print("Mine")
    print(result)
